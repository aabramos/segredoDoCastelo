 ...
 */
public class BotaoA extends Construcao {
	 ...
	 * @param y
	 */
	public BotaoA(double x, double y) {
		super(x, y);
        imortal = true;
        bloqueia = false;
        // Botao está no chão
	...
	public void frame() {
		...
		if (porta == null){
			porta = (new Porta(32 * (nivel.larguraNivel / 2 - .5), (nivel.alturaNivel - 33.5) * 32));
		...
		for (Entidade e : entities) {
			...
			if (e instanceof LetraA && status  == 0) {
				nivel.removerEntidade(porta);
				nivel.removerEntidade(e);
				Main.sons.reproduzirSom("/sons/botao.wav", (float) pos.x, (float) pos.y);
				nivel.adicionarEntidade(new Aestatico(32 * (nivel.larguraNivel / 2 - 2.5), (nivel.alturaNivel - 28.75) *  32));
