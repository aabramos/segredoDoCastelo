...
/**
 * A classe BotaoR, que cria um botão ativado por LIBRAS.
 */
public class BotaoR extends Construcao {
	 ...
	 * @param y 
	 */
	public BotaoR(double x, double y) {
		super(x, y);
        imortal = true;
        bloqueia = false;
        // Botao está no chão
	...
	public void frame() {
		...
		if (porta == null){
			porta = (new Porta(32 * (nivel.larguraNivel / 2 - .5), (nivel.alturaNivel - 35.5) * 32));
		...
		for (Entidade e : entities) {
			if (e instanceof LetraR && status == 0) {
				...
				nivel.adicionarEntidade(new Restatico(32 * (nivel.larguraNivel / 2 - 0.52), (nivel.alturaNivel - 28.75) * 32));
